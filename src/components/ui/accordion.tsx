...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex flex-1 items-center justify-between py-4 text-left font-display font-semibold text-lg transition-all hover:text-accent [&[data-state=open]>svg]:rotate-180 cursor-pointer',
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn('pb-4 pt-1 text-muted text-base leading-relaxed', className)}>
      {children}
    </div>
  </AccordionPrimitive.Content>
));
